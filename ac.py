...
class GetTxt:         
    def __init__(self):
        with open("/home/aiker/Desktop/新建文本 1.txt","r") as f:
            self.txt=f.read()
        with open("/home/aiker/Desktop/stoplist.txt","r") as g:
            self.m =g.readlines()
            self.m = str(self.m).replace("\\n","")
            #self.m =re.sub("[\\n']","",str(self.m))
            self.m = self.m.replace("' ',","")
            self.m = self.m.replace("'","") 
            self.m = self.m.replace(" ","")
            self.m = self.m.replace(",,","")
            self.m = self.m.replace("''","")
            self.m = self.m.strip("[]").split(",")
           # print(self.m)
